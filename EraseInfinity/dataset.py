...
# 设置 NLTK 数据目录为 EraseInfinity 目录下的 nltk_data
# 这样 wordnet 会保存在项目目录中，不会因为 .cache 清理而丢失

#__file__：当前正在被执行的源文件的绝对路径
#os.path.abspath(__file__)：把 __file__ 再转一次绝对路径，防止 __file__ 本身是相对路径（某些启动方式下会出现）
#os.path.dirname(...)：去掉文件名，只保留目录部分
#结果：script_dir 就是“这段脚本所在目录”的绝对路径。
#用途：后面所有资源都打算放在“跟脚本同级的 nltk_data”里，实现“绿色便携包”
script_dir = os.path.dirname(os.path.abspath(__file__))
#得到 “…/EraseInfinity/nltk_data” 这样一个字符串，但还没创建文件夹
nltk_data_dir = os.path.join(script_dir, 'nltk_data')
#os.makedirs递归创建多级目录，若父目录不存在也一并创建，而os.mkdir只能创建一层
...
# 检查 wordnet 是否已经存在（目录或 zip 文件）
wordnet_exists = os.path.exists(wordnet_dir) and os.path.isdir(wordnet_dir)

if not wordnet_exists:
    # 检查是否有 wordnet.zip 文件（可能在 .cache 目录或其他位置）
    cache_wordnet_zip = os.path.join(os.path.expanduser("~"), ".cache", "nltk_data", "corpora", "wordnet.zip")
    
    if os.path.exists(wordnet_zip_path):
        # 如果当前目录下有 wordnet.zip，解压它
        print(f"发现 wordnet.zip，正在解压到 {wordnet_dir}...")
        try:
            ...
            print(f"解压 wordnet.zip 失败: {e}")
    elif os.path.exists(cache_wordnet_zip):
        # 如果 .cache 目录下有 wordnet.zip，复制并解压
        print(f"发现 .cache 目录下的 wordnet.zip，正在复制并解压...")
        try:
            shutil.copy2(cache_wordnet_zip, wordnet_zip_path)
            with zipfile.ZipFile(wordnet_zip_path, 'r') as zip_ref:
                zip_ref.extractall(os.path.join(nltk_data_dir, 'corpora'))
            print("wordnet 复制并解压完成")
            wordnet_exists = True
        except Exception as e:
            print(f"复制并解压 wordnet.zip 失败: {e}")

if not wordnet_exists:
    # 如果都不存在，下载 wordnet
    print("wordnet不存在，开始下载到 EraseInfinity/nltk_data...")
    try:
