...
#修改后
from infinity.models.basic import AdaLNBeforeHead, CrossAttnBlock, SelfAttnBlock, CrossAttention, FastRMSNorm, precompute_rope2d_freqs_grid
...
class Infinity(nn.Module):
    def __init__(
        ...
        #modify3
        #由于我们禁用了 flash_attn_func 的导入,故此段有关五个flash_attn_func的内容都会报错
        '''
        ...
        #modify4
        #flash_fused_op_installed被禁止导入
        #这段代码的作用是，在分布式训练环境中验证所有进程是否一致地安装了flash_fused_op,避免因部分进程支持部分进程不支持而导致的不一致问题
        #新增一句
        flash_fused_op_installed=False

        #这行代码创建一个形状为 (总进程数,) 的全 0 张量 t，并将其放在当前进程的设备上。
        t = torch.zeros(dist.get_world_size(), device=dist.get_device())
        #dist.get_rank()：获取当前进程的序号（0 到 总进程数 - 1，例如 8 卡训练时为 0~7）
        t[dist.get_rank()] = float(flash_fused_op_installed)
        #阻塞所有进程，直到所有进程都执行到这一步，确保后续操作在所有进程同步后进行（避免因进程进度不一致导致的数据错乱）。
        dist.barrier()
        #allreduce是分布式通信操作，会将所有进程的t张量按元素求和，并将结果广播到所有进程
        #若总进程数为 8，其中 3 个进程安装了 flash_fused_op，则所有进程的 t 会变成一个包含 3 个 1.0、其余为 0.0 的张量（求和后）。
        dist.allreduce(t)
        #t.sum().item()：计算聚合后张量 t 的总和（即所有进程中安装了 flash_fused_op 的数量）
        #断言总和必须是 0（所有进程都未安装）或 总进程数（所有进程都安装了）
        #若不满足，则抛出错误，提示 flash_fused_op 状态不一致（部分进程安装，部分未安装）
